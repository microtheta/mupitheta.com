...
def main():
    ...
    if arguments['generate']:
        ...
        #command = "find /static -type f -exec sed -i 's/http:\/\/localhost:2368/https:\/\/microtheta.com/g' {} +"
        command = "find ./static -type f -name \"*.html\" -print0 | xargs -0 sed -i '' -e 's/localhost:2368/microtheta.com/g'"
        os.system(command)
        command = "find ./static -type f -name \"*.html\" -print0 | xargs -0 sed -i '' -e 's/%3F/?/g'"
        os.system(command)
        ...
        repo.git.execute(['git', 'push', '-u', '-f', origin.name,
